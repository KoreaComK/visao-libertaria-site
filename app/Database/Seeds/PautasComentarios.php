...
class PautasComentarios extends Seeder
{
	public function run()
	{
		...
		for ($artigos_id = 1; $artigos_id < 1000; $artigos_id++) {
			...
			$r = $this->db->query('SELECT id FROM pautas')->getResult();
			...
			for ($j = 1; $j < $rand; $j++) {
				...
				$this->db->table('pautas_comentarios')->insert($data);
